...
Table explaining each file:

| File | Purpose | Lifecycle |
|------|---------|-----------|
| PROTOCOL.md | Router - tells agent which workflow to load | Immutable template |
| WORK.md | Session state + detailed log | Perpetual - grows over time, housekeeping archives |
| INBOX.md | Open questions and loops | Cleared when loops resolved |
...
- Artifact overview table present
...
These principles MUST be preserved in all iterations:

1. **Pair programming > task execution:** Agent proposes and challenges, doesn't just execute
2. **Perpetual WORK.md:** Never delete session history, only archive
3. **Grep-first retrieval:** All content must be discoverable via single-line patterns
4. **User controls completion:** Agent signals readiness, user decides when to archive/PR
5. **First-turn conversation:** Agent talks before writing to artifacts
6. **Confirmation loops:** Every response ends with explicit handoff to user
