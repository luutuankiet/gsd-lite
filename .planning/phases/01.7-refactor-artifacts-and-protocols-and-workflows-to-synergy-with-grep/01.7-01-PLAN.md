...
4. **NEW: File Reading Strategy section (ADD AFTER Fresh Agent Resume Protocol):**
   ...
   Always grep before reading large artifacts. Pattern:

   1. **Discover structure:** `grep "^## " WORK.md` → returns section headers with line numbers
   2. **Calculate line ranges:** Section ends at (next header line - 1) or EOF
   3. **Surgical read:** Read only the section you need
   ...
   **Line range calculation:**
   If grep returns: `15: ## Key Events Index` and `45: ## Session Log`
   Then Key Events Index spans lines 15-44.

   **Fallback (no grep tool):**
...
- No references to ephemeral WORK.md
  ...
  <done>
PROTOCOL.md updated with merged file structure, new router, grep-first behavior prompting, and perpetual WORK.md lifecycle.
  </done>
</task>
