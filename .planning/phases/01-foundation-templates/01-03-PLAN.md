...
4. **Checkpoint Types** (reference CONTEXT.md decisions):
   - **📫 Loop captured**: Informational, loop proposed and approved
   - **⚡ Decision made**: Informational, user selected option
   - **✅ Task complete**: Informational, unit of work done
   - **🔮 Phase complete**: Informational, milestone reached
   - **🧪 Hypothesis validated/invalidated**: Informational, test result
   - **🛑 Blocking**: Requires explicit user action before proceeding (aggressive emoji wall: 🛑🛑🛑🛑🛑🛑🛑)
...
5. **Visual Conventions**:
   ...
   - Blocking barriers: 🛑🛑🛑 (7+ emojis) with horizontal rules
...
6. **Available Actions Menu**:
   ...
   - Format: Single-line shortcut list: 📋 /continue | /pause | /discuss
...
7. **Context Stack** (for forking):
   ...
   - Example: Working TASK-003 → /discuss CB invoices → return to TASK-003
...
4. **Protocol section** (summary, link to full reference):
   ...
   - See: .planning/templates/PROTOCOL_REFERENCE.md for complete protocol
5. **Templates section**:
   - Location: .planning/templates/
   - Entry point: README.md
   - Session artifacts: .project/sessions/YYYY-MM-DD-description/
6. **Workflows section** (high-level):
   - **Loop capture**: Agent proposes (XML) → user approves → write to LOOPS.md
   - **Context scoping**: Start narrow → track budget → exclude deliberately → document rationale
   - **Session state**: Update STATE.md after each turn → show in sticky note → recovery via "update artifacts"
...
7. Valid AGENTS.md format (check against standard)
  ...
  <done>
AGENTS.md exists with cross-platform configuration, commands, protocol summary, and template references
  </done>
</task>
...
3. Cross-references: AGENTS.md → PROTOCOL_REFERENCE → Templates (full chain)
...
- Cross-references work: AGENTS.md → PROTOCOL_REFERENCE → Templates
</success_criteria>
