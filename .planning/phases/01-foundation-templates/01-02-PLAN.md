...
3. **Session initialization protocol**:
   - Check for existing STATE.md, LOOPS.md, CONTEXT.md (if exist → resume, if not → create)
...
5. **Protocol checklist** (agent's self-reminder - embeds in sticky note behavior):
   ...
   - When loop identified: Propose loop in XML → wait for approval
   - When checkpoint reached: Use visual barrier (🛑🛑🛑) → state action required
   - When artifact updated: Show in sticky note UPDATED field
6. **Artifact references**: Where to find STATE, LOOPS, CONTEXT templates (link to .planning/templates/)
...
8. **Recovery from drift**: If user says "update artifacts" → read STATE.md, show current state, resume protocol
...
10. **Example session**: Show first 3 agent responses with sticky notes (initialization → task work → loop capture)
...
7. grep '```gsd-status' matches sticky note template marker
  ...
  <done>
BOOTLOADER_TEMPLATE.md exists with sticky note protocol, checklist for agent compliance, dual workflow support, and example session
  </done>
</task>
...
4. **GTD export format**: TickTick-compatible structure:
   ...
   - Open loops → Next actions (with context)
   - Clarifying loops → Waiting for (with question)
...
2. **Quick start**: 3-step workflow:
   - Step 1: Copy BOOTLOADER_TEMPLATE → start session
...
4. **Workflow diagram** (ASCII/markdown):
   ```
   Session Start → BOOTLOADER (initialize)
                    ↓
   During Work   → STATE (track current)
                    ├→ LOOPS (capture questions)
                    └→ CONTEXT (manage budget)
                    ↓
   Session End   → SUMMARY (export to GTD)
   ```
...
2. BOOTLOADER_TEMPLATE.md: Sticky note template copy-pasteable, protocol checklist visible
